...
COLORS = ['black', 'white', 'grey', 'green', 'snow', 'ghost white', 'white smoke', 'gainsboro', 'floral white', 'old lace',
...
def get_polygon():
	...
	while type(sides) != int:
		try:
			...
			sides = screen.textinput(title='Sides', prompt='How many sides?')

	while type(pen_size) != int:
		try:
			pen_size = int(pen_size)
		except:
			pen_size = screen.textinput(title='Pen size', prompt='Size of the the pen (in pixels):')

	while type(size) != int:
		try:
			size = int(size)
		except:
			size = screen.textinput(title='Size', prompt='Size of the sides (in pixels):')

	while pen_color not in COLORS:
		pen_color = screen.textinput(title='Color', prompt='Color of the polygon sides:')

	while filled not in VALID_FILL_VALUES:
...
get_polygon()
draw_polygon(sides, size, pen_size, pen_color, filled, fill_color, bg_color)
